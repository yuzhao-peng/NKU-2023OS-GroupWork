...
load_icode(int fd, int argc, char **kargv) {
 ...
 /* (1) create a new mm for current process 
    ...
    for (phnum = 0; phnum < elf->e_phnum; phnum ++) {
        ...
        // (3.3)  调用 mm_map 函数设置新的 VMA（ph->p_va, ph->p_memsz）
        vm_flags = 0, perm = PTE_U | PTE_V;  // 建立虚拟地址和物理地址之间的映射关系
        ...
        // (3.4) 分配内存，并将每个程序段的内容（from、from+end）复制到进程的内存（la、la+end）
        ...
        // (3.5) 调用 callpgdir_alloc_page，为 BSS 分配页面，在这些页面置 0
        if (start < la) {
    ...
    // (5) 设置当前进程的 mm、cr3，重置 pgidr（使用 lcr3 MARCO）
    mm_count_inc(mm);
    current->mm = mm;
