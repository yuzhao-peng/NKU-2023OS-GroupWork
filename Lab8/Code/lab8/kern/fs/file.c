...
// open file
// 实际上该函数给当前用户进程分配了一个file数据结构，但是没有对应的inode
int
file_open(char *path, uint32_t open_flags) {
    ...
    int ret;
    struct file *file;
    if ((ret = fd_array_alloc(NO_FD, &file)) != 0) {  // 在当前进程分配file descriptor（进程描述符）
        return ret;
    }
    struct inode *node;
