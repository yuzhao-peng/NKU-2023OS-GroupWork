 ...
 */
/* 
 * sfs_bmap_load_nolock - 根据 DIR 的 inode 和 inode 中区块的逻辑索引，查找磁盘区块的编号
 * @sfs：sfs 文件系统 
 * @sin：内存中的 sfs inode 
 * @index：inode 中磁盘区块的逻辑索引 
 * @ino_store：磁盘区块的编号 */
static int
sfs_bmap_load_nolock(struct sfs_fs *sfs, struct sfs_inode *sin, uint32_t index, uint32_t *ino_store) {
    ...
    assert(sfs_block_inuse(sfs, ino));
    if (create) {  // 保证写回时有空间可以使用，因为此时我们已经访问了这一部分的最后一块
        din->blocks ++;
    }
    if (ino_store != NULL) {
        *ino_store = ino;
 ...
 */
/* 
 * sfs_dirent_search_nolock - 读取 DIR 中的每个文件条目，将文件名与每个条目->名进行比较，如果相等，则返回槽位和该文件 inode 的磁盘编号 
 * @sfs：sfs 文件系统 
 * @sin：内存中的 sfs inode 
 * @name：文件名 
 * @ino_store：  该文件（带文件名）的 inode 的磁盘数目
 * @slot: 文件条目的逻辑索引（注意：每个文件条目占用一个磁盘块） 
 * @empty_slot: 空文件条目的逻辑索引。
 */
// 查找name返回inode编号和空闲entry
static int
...
sfs_io(struct inode *node, struct iobuf *iob, bool write) {
    ...
    lock_sin(sin);
    {
        ...
        ret = sfs_io_nolock(sfs, sin, iob->io_base, iob->io_offset, &alen, write);  // sfs_io_nolock函数进行读取文件操作
        if (alen != 0) {
